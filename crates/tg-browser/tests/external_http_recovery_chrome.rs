...
async fn send_stalled_response(
    ...
    timeout(HTTP_TIMEOUT, stream.write_all(response.as_bytes()))
        .await
        .map_err(|_| test_error("stalled loopback response write timed out"))??;
    events
...
async fn send_html_response(stream: &mut TcpStream, status: &str, body: &str) -> TestResult {
    ...
    timeout(HTTP_TIMEOUT, stream.write_all(response.as_bytes()))
        .await
        .map_err(|_| test_error("loopback recovery response write timed out"))??;
    timeout(HTTP_TIMEOUT, stream.shutdown())
        .await
        .map_err(|_| test_error("loopback recovery response shutdown timed out"))??;
    Ok(())
}
